...
const SPAWN_BUBBLE_SLOTS = Array.from({ length: SPAWN_BUBBLE_COUNT }, (_, i) => i);
...
function SpawnBubble({ slot, depth, speed, controlRef }: SpawnBubbleProps) {
  ...
  const reset = (elapsedTime: number) => {
    ...
    data.y = MathUtils.randFloat(-height * 0.5, 0) - BUBBLES_GROUP_Y_OFFSET;
    data.z = z;
    data.scale = scale;
    data.exitLimit = height * 2;
    data.speedFactor = getBubbleSpeedFactor(scale);
  };
...
function Bubbles({ speed = 3, count = 80, depth = 30 }) {
  ...
  useFrame((state, delta) => {
    ...
    if (elapsedTime >= control.nextSpawnTime) {
      const openSlots = control.activeSlots
        .map((isActive, slot) => (!isActive ? slot : -1))
        .filter((slot) => slot !== -1);
  ...
  return (
    <group ref={ref}>
      <Instances limit={count + SPAWN_BUBBLE_COUNT} castShadow receiveShadow position={[0, BUBBLES_GROUP_Y_OFFSET, 0]}>
        <sphereGeometry args={[1, 64, 64]} />
        <meshStandardMaterial roughness={1} color="#f0f0f0" />
        {Array.from({ length: count }, (_, i) => (
          <Bubble key={i} index={i} z={MathUtils.lerp(0, depth, i / count)} speed={speed} />
        ))}
        {SPAWN_BUBBLE_SLOTS.map((slot) => (
          <SpawnBubble
            key={`spawn-${slot}`}
            slot={slot}
            depth={depth}
